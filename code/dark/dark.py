...
class Dark(ReferenceFile):
    ...
    def __init__(self, ramp_image_list, meta_data, bit_mask=None, outroot=None, clobber=False):

        self.outroot = outroot

        super(Dark, self).__init__(ramp_image_list, meta_data, bit_mask=bit_mask, clobber=clobber)
    ...
    def compute_dark_rate(self, warm_thresh=3, hot_thresh=5, unreliable_thresh=1,
                          warm_bit=12, hot_bit=11, unreliable_bit=23):

        logging.info('Computing sigma-clipped mean dark rate image')
        # Compute the mean and standard deviation of all of the darks in the input list.
        self.dark_rate_image, _, dark_noise = sigma_clipped_stats(self.data, axis=0)

        logging.info('Computing sigma-clipped median and standard deviation of mean dark rate image')
        # Compute the median and standard deviation of the average rate image.
        _, dark_rate_med, dark_rate_std = sigma_clipped_stats(self.dark_rate_image)
        ...
        logging.info(f'\tWarm pixel threshold = {dark_rate_med + (warm_thresh * dark_rate_std):0.5f} ADU / sec')
        logging.info(f'\tHot pixel threshold = {dark_rate_med + (hot_thresh * dark_rate_std):0.5f} ADU / sec')
        logging.info(f'\tUnreliable pixel threshold = {unreliable_thresh:0.5f} ADU / sec')
        ...
        warm_pixel = np.where((self.dark_rate_image >= (dark_rate_med + (warm_thresh * dark_rate_std))) &
                              (self.dark_rate_image < (dark_rate_med + (hot_thresh * dark_rate_std))))

        hot_pixel = np.where(self.dark_rate_image >= (dark_rate_med + (hot_thresh * dark_rate_std)))
    ...
    def make_dark(self, n_reads=1, n_resultants=1, frame_time=2.806, ma_table_name='ALL'):
        ...
        # Calculate the dark image for each resultant. Each resultant is treated
        # as the average of the reads in it, so calculate the dark image for each read
        # and then combine them. If using all the reads, don't average them.
        ...
        for i in range(n_resultants):
            ...
            for j in range(n_reads):
                reads[j, :, :] = self.dark_rate_image * (j + 1 + (n_reads * i)) * frame_time
            dark_image[i, :, :] = np.mean(reads, axis=0) if n_reads > 1 else np.squeeze(reads)

        logging.info('Constructing dark datamodel')
        # Construct the dark object from the data model.
        dark_asdf = DarkModel(data=dark_image, err=np.zeros((n_resultants, 4096, 4096), dtype=np.float32),
                              dq=self.mask)
