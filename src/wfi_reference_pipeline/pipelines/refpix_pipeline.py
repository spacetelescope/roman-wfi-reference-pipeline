...
class RefPixPipeline(Pipeline):
    ...
    def run_pipeline(self, file_list=None):
        ...
        out_file_path = self.file_handler.format_pipeline_output_file_path(
            "WIM", tmp.meta_referencepixel.instrument_detector
        )

        rfp_refpix = ReferencePixel(
            meta_data=tmp.meta_referencepixel,
            file_list=file_list,
            ref_type_data=None,
        ...
        rfp_refpix.outfile = self.file_handler.format_pipeline_output_file_path(
            "WIM", rfp_refpix.meta_data.instrument_detector
        )
