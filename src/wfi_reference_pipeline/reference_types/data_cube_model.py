...
class DataCubeModel():
    """
    CubeToImage class to reduce cubes into images for reference types
    ...
    def __init__(self, data_cube, wfi_type):
        ...
        self.time_array = None  # WIM/WSM dependent time array populated by the number of reads and multiplied by mode specific frame time for each read
        self.wfi_mode = None # WIM or WSM
        ...
        logging.debug(f'Creating exposure time array {self.nr_reads} reads long with a frame '
                     f'time of {self.frame_time} seconds.')
        self.time_arr = np.array([self.frame_time * i for i in range(1, self.nr_reads + 1)])
    ...
    def _fit_ramp_model(self, order=1):
        ...
        TODO - Algorith on how to incorporate "order"?

        """
        logging.debug('Making ramp model for the input read cube.')
        ...
        try:
            p, v = np.polyfit(self.time_arr, self.data_cube.reshape(len(self.time_arr), -1), 1, full=False, cov=True)
            ...
            self.ramp_model = np.zeros((self.n_reads, self.nr_pixels, self.nr_pixels), dtype=np.float32)
            for tt in range(0, len(self.time_arr)):
                # Construct a simple linear model y = m*x + b.
                self.ramp_model[tt, :, :] = self.rate_image * self.time_arr[tt] + self.intercept_image
