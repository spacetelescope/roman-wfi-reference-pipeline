...
class PixelFlat(ReferenceType):
    """
    Class PixelFlat() inherits the ReferenceType() base class methods where
    static meta data for all reference file types are written.

    The class PixelFlat() ingests a list of files and finds all exposures with the same filter
