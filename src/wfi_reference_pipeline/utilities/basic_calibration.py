...
class CalDCL:
    ...
    def subtract_reference_pixels(self):
        ...
        self.ref_pix = np.concatenate((self.all_reads[:, :, :4],
                                       self.all_reads[:, :, -4:]), axis=2)
        new_shape = (self.all_reads.shape[0], self.all_reads.shape[1], 1)
        self.mean_ref_pix = np.mean(self.ref_pix, axis=2,
                                    dtype=np.int32).reshape(new_shape)

        logging.info('Row-subtracting reference pixel mean')

        # Subtract off the mean reference pixel per row.
        self.cal_reads = self.all_reads[1:] - self.mean_ref_pix[1:]
        self.zero_read = self.all_reads[0] - self.mean_ref_pix[0]
    ...
    def ramp_fit(self):
        ...
        self.ramp_image = slopes.reshape(cube_shape[1],
                                         cube_shape[2]).astype(np.float32)
