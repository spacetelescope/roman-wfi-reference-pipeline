...
def _find_config_file(config_filename, config_path=""):
    ...
    """
    # User sent in config_path specifically
    if len(config_path):
...
def _get_config(config_filename, config_path=""):
    ...
    """
    config_file_location = _find_config_file(config_filename, config_path=config_path)
...
def get_logging_config(config_file="config.yml", config_path=""):
    ...
    settings = _get_config(config_file, config_path)
    _validate_config(settings, CONFIG_SCHEMA)
    return settings["logging"]


def get_data_files_config(config_file="config.yml", config_path=""):
...
def get_pipelines_config(ref_type, config_file="pipelines_config.yml", config_path=""):
...
def get_quality_control_config(ref_type, config_file="quality_control_config.yml", config_path=""):
    ...
    try:
        if ref_type == REF_TYPE_DARK:
            ...
            settings = _get_config(config_file, config_path)["readnoise_control"]
...
def get_crds_submission_config(config_file="crds_submission_config.yml", config_path=""):
    """
