...
def get_science_pixels_cube(data_cube, border=VIRTUAL_PIXEL_DEPTH):
    ...
    if num_i_pixels == num_j_pixels == DETECTOR_PIXEL_X_COUNT:
        return data_cube[:, border:-border, border:-border]
    elif num_i_pixels == num_j_pixels == SCI_PIXEL_X_COUNT:
        return data_cube
    else:
        raise ValueError(
...
def add_science_pixels_to_resource_slice(data_cube_slice, border=VIRTUAL_PIXEL_DEPTH):
    ...
    resource_slice = np.zeros(
        (DETECTOR_PIXEL_X_COUNT, DETECTOR_PIXEL_Y_COUNT), dtype=np.float32
    )
